...
public interface PropertyRepository extends CrudRepository<Property, String> {
	...
	@Query("select p from Property p where (p.price <= :price) and (p.type=:type) and (p.rooms=:rooms)")
	...
	@Query("select p from Property p where ((p.type=:type) and (p.rooms=:rooms)")
	...
	public List<Property> findByRooms(String rooms);
